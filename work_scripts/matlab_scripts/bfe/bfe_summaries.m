...
unique_sims = unique(all_simulation_bfe(1,:),'stable');
combined_across_sims_data_means = {};
combined_across_sims_data_variances = {};
combined_across_sims_data_mean = [];
combined_across_sims_data_std = [];
combined_across_sims_data_ste = [];
distribution_data_allsims = {};
...
for unique_sim = 1:length(unique_sims)
    ...
    distribution_data = single_mutation_data(:);
    distribution_data_allsims = cat(2,distribution_data_allsims,distribution_data);

    single_mutation_data_means = mean(single_mutation_data,1).';
    single_mutation_data_mean = mean(single_mutation_data_means);
    single_mutation_data_std = std(single_mutation_data_means,1);
    single_mutation_data_ste = single_mutation_data_std/sqrt(length(single_mutation_data(1,:)));
    

    single_mutation_data_variances = var(single_mutation_data).';


    combined_across_sims_data_means = cat(2,combined_across_sims_data_means,single_mutation_data_means);
    combined_across_sims_data_variances = cat(2,combined_across_sims_data_variances,single_mutation_data_variances);
    combined_across_sims_data_mean = cat(2,combined_across_sims_data_mean,single_mutation_data_mean);
    combined_across_sims_data_std = cat(2,combined_across_sims_data_std,single_mutation_data_std);
    combined_across_sims_data_ste = cat(2,combined_across_sims_data_ste,single_mutation_data_ste);
...
% scatter(pep_4_prot_triple,single_mutation{44}(:,4))
...
Summary_BFE_figure = figure('Name',Title,'NumberTitle','off');
fill(nan,nan,wellesley_blue,'FaceAlpha',0.3)
hold on;
fill(nan,nan,firebrick_red,'FaceAlpha',0.3)
hold on;
for_normalization_figure = [];
for_normalization_figure_all = [];
iter = 1;
iter_2 = 1;
A_meanpersim = zeros(6,16);

for sim_data = 1:length(combined_across_sims_data_means)
    current_number_of_sims = length(combined_across_sims_data_means{sim_data})/4;
    
    if (sim_data == 1 || sim_data == 3 || sim_data == 5  || sim_data == 7 || sim_data == 15)
        for_normalization_figure = [for_normalization_figure; distribution_data_allsims{sim_data},(ones(length(distribution_data_allsims{sim_data}),1).*iter)];
        iter = iter+1;
    end
    
    if (sim_data == 3 || sim_data == 5 || sim_data == 7  || sim_data == 9 || sim_data == 11 || sim_data == 13 || sim_data == 2 || sim_data == 15)
        for_normalization_figure_all = [for_normalization_figure_all; distribution_data_allsims{sim_data},(ones(length(distribution_data_allsims{sim_data}),1).*iter_2)];
        iter_2 = iter_2+1;
    end
    ...
    if sim_data == 1 %wt
        ...
        % n=101;
        % a = distribution_data_allsims{sim_data};
        ...
        std(b)
        ste = std(b)/sqrt(3);
        A_meanpersim(1:3,sim_data) = b;
    end


    %comment out the following block and comment in the later scatter
    %comment if you want all red dots
    start_pep = 1;
    end_pep = 4;
    color_increment = 1;
    % colors = [black;green;red;purple;brown;blue];
    % for current_sim = 1:current_number_of_sims
    %     scatter(sim_data,combined_across_sims_data_means{sim_data}(start_pep:end_pep,:),100,colors(color_increment,:),'.');
    %     start_pep = start_pep+4;
    %     end_pep = end_pep+4;
    %     color_increment = color_increment+1;
    %     hold on;
    % end
    
    modulus = mod(sim_data,2);
    if modulus == 1
        v = violinplot(sim_data,distribution_data_allsims{sim_data},DensityDirection="negative");
        hold on;
        violins = [violins,v];
        % daba = [distribution_data_allsims{sim_data},ones(length(distribution_data_allsims{sim_data}),1) *(1)];
        % scatter((sim_data)-0.15,mean(distribution_data_allsims{sim_data}))
        errorbar((sim_data)-0.25,mean(distribution_data_allsims{sim_data}), ste, '.','LineWidth',1.3,'MarkerSize',15, 'Color',black)
    elseif modulus == 0
        v = violinplot((sim_data-1),distribution_data_allsims{sim_data},DensityDirection="positive");
        hold on;
        violins = [violins,v];
        % daba = vertcat(daba,[distribution_data_allsims{sim_data},ones(length(distribution_data_allsims{sim_data}),1)*(2)]);
        % scatter((sim_data-1)+0.15,mean(distribution_data_allsims{sim_data}))
        errorbar((sim_data-1)+0.25,mean(distribution_data_allsims{sim_data}), ste, '.','LineWidth',1.3,'MarkerSize',15, 'Color',black)
        
        % b = boxchart((ones(length(daba),1) *(sim_data-1)),daba(:,1),'MarkerStyle','none',GroupByColor=daba(:,2));
        % boxplots = [boxplots,b];
    end

    hold on;
    % scatter(sim_data,combined_across_sims_data_means{sim_data},100,wellesley_purple,'.');
    hold on;
    

    %trying to look at influences in variability in data means

    inverse_variances = 1./combined_across_sims_data_variances{sim_data};
    weight = inverse_variances/sum(inverse_variances);
    weighted_mean = sum(combined_across_sims_data_means{sim_data}.*weight);
    % scatter(sim_data,weighted_mean,100,black,'.');
...
for vio = 1:length(violins)
    modulus = mod(vio,2);
    % 
    % boxplots(vio).BoxFaceColor = black;
    % boxplots(vio).BoxFaceAlpha = 0.05;
    % boxplots(vio).ColorGroupWidth = 0.4;
    % boxplots(vio).BoxEdgeColor = [0,0,0,0.02];
    % boxplots(vio).LineWidth = 0.4;
    % boxplots(vio).BoxWidth = 1.00;

    
    violins(vio).DensityWidth = 1.75;

    if modulus ~= 0
        violins(vio).FaceColor = wellesley_blue;
    else modulus = 0;
        violins(vio).FaceColor = firebrick_red;
    end
end
...
ylabel('Binding Free Energy ({\itkcal/mol})');
% xlabel('Simulation');
string_sims = strrep(upper(string(unique_sims)),'_',' ');
% string_sims = strrep(string_sims,'H','h');
% string_sims = strrep(string_sims,'h T1R A6R L8R','hT1R A6R L8R');
xticks(1:2:length(string_sims));
xticklabels(string_sims(1:2:end));
blog = legend('Deprotonated','Protonated');

fitting_data(tight_fitting=true, data_spacing_x=0.025);
consistent_figures(figure_name=Summary_BFE_figure, rotate_x_labels_by_angle=20, PDF_PNG_name=Title,legend=blog);
ylim([-45,0])
...
wt_matrix = repmat(A(:,1),1,16);
[h1,p1]=ttest2(wt_matrix,A);
ttest(A(:,1),A(:,3))

save('bfe_fornorm.mat',"for_normalization_figure")
save('bfe_fornorm_all.mat',"for_normalization_figure_all")

A_meanpersim(A_meanpersim == 0) = NaN;
wt_matrix = repmat(A_meanpersim(:,1),1,16);
[h2,p2]=ttest2(wt_matrix,A_meanpersim);
all_sig = [h1;p1;h2;p2]

% figure(Summary_BFE_figure)
% groups = {[1,3],[1,6],[1,10],[1,13],[1,15],[1,16]};
% p_groups = [0.015084749640525,0.025745384203039,0.046265795579201,0.021124638566288,0.000203045604207,0.012700386046404];
% h=sigstar(groups,p_groups)
%%

% y_upper = 0;
% for group = 1:length(p_groups)
%     % spacer = y_upper-0.5*(length(p_groups)-group);
%     spacer = 10
%     for bracket = 1:4
% 
%         Y=get(h(group,1),'YData'); Y(bracket)=Y(bracket)-80; set(h(group,1),'YData',Y);
%         Y=get(h(group,2),'YData'); Y(bracket)=Y(bracket)-80; set(h(group,2),'YData',Y);
%     end
% end
...
% all_simulation_slope = [];
% all_simulation_mindist = [];
% 
% pca_all_bfe = [];
% pca_all_hbond = [];
% pca_all_hbond2 = [];
...
%         current_pep_backmindist = load("../analysis/"+all_sim_names_sorted{sim}+"_analysis/mindist/"+pep+"p_residue_backbone_mindist_summary.txt");
% 
% 
...
%         order_array = 1:1:21;
%         current_pep_backmindist = current_pep_backmindist(2:22,1:25:2501).';
%         % current_pep_backmindist_mean_across_res = cat(1,order_array,mean(current_pep_backmindist));
%         current_pep_backmindist_mean_across_snapshot = mean(current_pep_backmindist,2);
% 
%         snapshot_slope = [];
%         for row = 1:length(current_pep_backmindist(:,1))
%             p = polyfit(order_array,current_pep_backmindist(row,:),1);
%             snapshot_slope = cat(1,snapshot_slope,p(1,1));
%         end
%         % p = polyfit(current_pep_backmindist_mean_res(1,:),current_pep_backmindist_mean_res(2,:),1) ;
%         % current_pep_backmindist_mean_all = [mean(current_pep_backmindist_mean_res(2,:),2) , p(1,1)];
% 
% 
%         current_sim_bfe = cat(2,current_sim_bfe,current_pep_bfe);
...
% 
%         current_sim_slope = cat(2,current_sim_slope,snapshot_slope);
%         pca_all_slope = cat(1,pca_all_slope,snapshot_slope);
% 
%         current_sim_mindist = cat(2,current_sim_mindist,current_pep_backmindist_mean_across_snapshot);
%         pca_all_mindist = cat(1,pca_all_mindist,current_pep_backmindist_mean_across_snapshot);
%     end
% 
%     current_sim_bfe_cell = {current_sim_name{1},current_sim_bfe}.';
%     all_simulation_bfe = cat(2,all_simulation_bfe,current_sim_bfe_cell);
...
% 
%     current_sim_slope_cell = {current_sim_name{1},current_sim_slope}.';
%     all_simulation_slope = cat(2,all_simulation_slope,current_sim_slope_cell);
%     current_sim_mindist_cell = {current_sim_name{1},current_sim_mindist}.';
%     all_simulation_mindist = cat(2,all_simulation_mindist,current_sim_mindist_cell);
% 
% end
% 
% 
...
% combined_across_sims_mindist_data_means = {};
% combined_across_sims_mindist_data_mean = [];
% 
% fprintf("bfe,memb,peps,itself,water,mindist\n")
% % pca_data_object = cat(2,pca_all_hbond,pca_all_hbond2,pca_all_hbond3,pca_all_hbond4,pca_all_mindist);
% pca_data_object = cat(2,pca_all_hbond,pca_all_hbond2,pca_all_hbond3,pca_all_hbond4);
% mapcaplot(pca_data_object)
...
%     logical_single_mutation_slope = matches(all_simulation_slope(1,:),unique_sims{unique_sim});
%     logical_single_mutation_mindist = matches(all_simulation_mindist(1,:),unique_sims{unique_sim});
% 
% 
...
% 
%     single_mutation_slope = cell(size(all_simulation_slope(1,:)));
%     single_mutation_slope(logical_single_mutation_slope) = all_simulation_slope(2,logical_single_mutation_slope);
%     single_mutation_data_slope = cat(2,single_mutation_slope{:});    
% 
%     single_mutation_mindist = cell(size(all_simulation_mindist(1,:)));
%     single_mutation_mindist(logical_single_mutation_mindist) = all_simulation_mindist(2,logical_single_mutation_mindist);
%     single_mutation_data_mindist = cat(2,single_mutation_mindist{:});    
% 
% 
%     figure;
%     % model = fitlm(reshape(single_mutation_data_hbond,[],1),reshape(single_mutation_data_bfe,[],1))
%     scatter3(single_mutation_data_hbond3,single_mutation_data_bfe,single_mutation_data_mindist);
%     xlabel('Hbonds to peps');
%     ylabel('Binding Free Energy ({\itkcal/mol})');
%     zlabel('Average mindist to peptide');
...
%     single_mutation_data_hbond3_means = mean(single_mutation_data_hbond3,1).';
%     single_mutation_data_hbond3_mean = mean(single_mutation_data_hbond3_means);
%     single_mutation_data_mindist_means = mean(single_mutation_data_mindist,1).';
%     single_mutation_data_mindist_mean = mean(single_mutation_data_mindist_means);
% 
...
%     combined_across_sims_hbond3_data_means = cat(2,combined_across_sims_hbond3_data_means,single_mutation_data_hbond3_means);
%     combined_across_sims_hbond3_data_mean = cat(2,combined_across_sims_hbond3_data_mean,single_mutation_data_hbond3_mean);
%     combined_across_sims_mindist_data_means = cat(2,combined_across_sims_mindist_data_means,single_mutation_data_mindist_means);
%     combined_across_sims_mindist_data_mean = cat(2,combined_across_sims_mindist_data_mean,single_mutation_data_mindist_mean);
% 
% end
% 
% 
...
%      scatter3(combined_across_sims_hbond3_data_means{sim_data}, combined_across_sims_bfe_data_means{sim_data},combined_across_sims_mindist_data_means{sim_data});
%      hold on;
% end
% 
% 
